        ...
        <div className="space-y-4 animate-pulse">
            {[...Array(count)].map((_, i) => (
                <div key={i} className="flex items-center space-x-4">
                    {type === 'card' && <div className="rounded-full bg-gray-200 h-16 w-16"></div>}
...
const ScrollProgress = () => {
    ...
    return (
        <div className="fixed top-20 left-0 h-1.5 bg-gray-100 w-full z-20">
            <div className="h-full bg-emerald-500 transition-all duration-100 ease-out shadow-lg" style={{ width: `${width}%` }}></div>
        </div>
    );
};
